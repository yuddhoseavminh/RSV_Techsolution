...
import { Bell, CreditCard, FolderKanban, LayoutDashboard, LifeBuoy, LogOut, UserRound } from "lucide-react";
        ...
        <header className="border-b border-slate-200 bg-white">
          <div className="section-shell flex h-[72px] items-center justify-between">
            <Link href="/" className="flex items-center gap-3">
              <span className="flex h-10 w-10 items-center justify-center rounded-lg bg-brand-slate text-[11px] font-bold text-white">
                RVS
              </span>
              ...
              {portalNav.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    className={cn(
                      "flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-slate-700 transition hover:bg-slate-100 hover:text-brand-blue",
                      pathname === item.href && "bg-blue-50 text-brand-blue"
                    )}
                  >
